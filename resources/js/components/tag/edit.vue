          ...
          <div class="row">
            <div class="col-md-12">
              <div class="form-group">
                ...
                <input type="text" class="form-control" id="example3cols1Input" placeholder="Name" v-model="form.tag_name">
                <small class="text-danger" v-if="errors.tag_name">{{ errors.tag_name[0] }}</small>
              </div>
            </div>     
          </div>
          <button class="btn btn-primary" style="margin-top:15px;" type="submit">Save Tag</button>
        </form>  
        </div>
    </div>
</div>
    ...
    export default {
      mounted(){
        ...
        methods:{   
          tagUpdate(){
            let id = this.$route.params.id
            axios.post('/api/update-tag/'+id,this.form)
            .then(() => {
