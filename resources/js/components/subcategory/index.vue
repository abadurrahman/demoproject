 ...
 <div style="padding:25px;margin-top:-5.5rem">
   <div class="card" style="padding:25px;">
            ...
            <div class="card-header">
              ...
              <router-link to="/add-subcategory" class="btn btn-sm btn-primary float-right">Add-SubCategory</router-link>
               ...
               <table class="table table-flush dataTable table table-hover" id="datatable-basic" role="grid" aria-describedby="datatable-basic_info">
                  <thead class="thead-light">
                   <tr role="row">
                     ...
                     <th class="sorting_asc" style="width: 153px;">SubCategory Name:</th>
                     <th class="sorting_asc" style="width: 153px;">Created At:</th>
                     <th class="sorting_asc" style="width: 153px;">Action:</th>   </tr>
                   </thead>
          
                <tbody>                 
                 <tr role="row" class="odd" v-for="(subcategory,index) in filtersearch" :key="subcategory.id">
                    <td>{{index+1}}</td>
                    <td>{{ subcategory.subcategory_name }}</td>
                    <td>{{ subcategory.created_at }}</td>
                    <td>
                      <router-link :to="{name: 'edit-subcategory', params:{id: subcategory.id} }" class="btn btn-sm btn-info" href="">Edit</router-link>
                      <a class="btn btn-sm btn-danger" style="color:white;" @click="deleteSubCategory(subcategory.id)">Delete</a>
                    </td>
                 </tr>
                </tbody>
              </table>
    ...
    export default {
      mounted(){
       ...
       computed:{
        ...
        methods:{
          allSubCategory(){
            axios.get("/api/all-subcategory?page=" + this.pagination.current_page)
           .then(response => {
